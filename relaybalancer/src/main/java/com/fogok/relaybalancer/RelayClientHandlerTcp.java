...
public class RelayClientHandlerTcp extends BaseTcpHndlrToClntInSvcToSvc<RelayConfig, TokenFromClientReader, ConnectorToAuthService> {
    ...
    public void handlerAdded(ChannelHandlerContext ctx) {
        info(String.format("Client %s joined to %s service", ctx.channel().remoteAddress(), RelayClientHandlerTcp.class.getSimpleName()));
    }
    ...
    private void connectToAuthService(ChannelHandlerContext ctx, Object msg) throws InvalidPropertiesFormatException {
        connectorToService.connectServiceToService(new BaseConnectorInSvcToSvc.ConnectToServiceCallback() {
            ...
            public void success(ChannelFuture channelFuture, BaseTcpHandlerInSvcToSvc svcToSvcHandler){
                tokenFromClientReader.setSrvToSrvHandler((RelayToAuthHandlerTcp) svcToSvcHandler);
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) {
        info(String.format("Client %s left in %s service", ctx.channel().remoteAddress(), RelayClientHandlerTcp.class.getSimpleName()));
    }
}
